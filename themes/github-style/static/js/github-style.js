...
function monthly(year, month, posts) {
  ...
  // 优化 12.3: Sort all items by date (most recent first)
  monthPosts.sort((a, b) => b.date - a.date);
  
  // Group consecutive items by type (posts vs inputs)
  const timelineGroups = [];
  let currentGroup = null;
  
  for (const item of monthPosts) {
    const isInput = ['blogs', 'books', 'watches', 'podcasts'].includes(item.section);
    const itemType = isInput ? 'inputs' : 'posts';
    
    // Start a new group if type changes or it's the first item
    if (!currentGroup || currentGroup.type !== itemType) {
      currentGroup = {
        type: itemType,
        items: []
      };
  ...
  // Generate timeline items for each group
  for (const group of timelineGroups) {
    if (group.type === 'posts') {
      // Generate Posts TimelineItem
      let liHtml = '';
          ...
          <details class="Details-element details-reset" open>
            <summary role="button" class="btn-link f4 muted-link no-underline lh-condensed width-full">
              <span class="color-text-primary ws-normal text-left">
                Created ${group.items.length} post${group.items.length > 1 ? 's' : ''}
              </span>
      ...
      // Generate Inputs TimelineItem
      let liHtml = '';
      for (const item of group.items) {
        // 优化 12.4: 使用 externalUrl 如果存在
        ...
        <time  title="This item was finished on ${months[item.date.getMonth()]} ${item.date.getDate()}"
          class="col-2 text-right f6 text-gray-light pt-1">
      ...
      <div class="TimelineItem ">
        <div class="TimelineItem-badge ">
          <svg class="octicon octicon-project" viewBox="0 0 16 16" version="1.1" width="16" height="16">
        ...
        <div class="TimelineItem-body ">
          <details class="Details-element details-reset" open>
            <summary role="button" class="btn-link f4 muted-link no-underline lh-condensed width-full">
              <span class="color-text-primary ws-normal text-left">
                Finished ${group.items.length} item${group.items.length > 1 ? 's' : ''}
              </span>
              <span class="d-inline-block float-right color-icon-secondary">
                <span class="Details-content--open float-right">
                  <svg class="octicon octicon-fold" viewBox="0 0 16 16" version="1.1" width="16" height="16">
...
function graph(year, posts, startDate, endDate) {
  const postsStr = posts.length === 1 ? "record" : "records";
  if (year === now.getFullYear().toString()) {
  ...
  let html = ``;
  const count = {};
  const dateDetails = {};  // Store section types for each date
  for (const post of posts) {
    ...
    if (count[date] === undefined) {
      count[date] = 1;
      dateDetails[date] = [post.section];
    } else {
      count[date]++;
      dateDetails[date].push(post.section);
    }
  }
  window.dateDetails = dateDetails;  // Make it globally accessible
  const monthPos = [];
  let startMonth = -1;
...
function svgTip(elem, count, dateStr) {
  ...
  if (count) {
    // Get section details for this date
    const sections = window.dateDetails && window.dateDetails[dateStr] ? window.dateDetails[dateStr] : [];
    const sectionCounts = {};
    ...
    for (const [section, cnt] of Object.entries(sectionCounts)) {
      ...
      const plural = cnt > 1 ? (name === 'watch' ? 'es' : 's') : '';
      parts.push(`${cnt} ${name}${plural}`);
    }
    
    const displayText = parts.length > 0 ? parts.join(', ') : `${count} records`;
    svgElem.innerHTML = `<strong>${displayText}</strong> on ${dateFmt}`;
  } else {
    svgElem.innerHTML = `<strong>No records</strong> on ${dateFmt}`;
  }
